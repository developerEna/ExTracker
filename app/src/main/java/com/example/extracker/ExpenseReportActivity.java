...
public class ExpenseReportActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        reference1 = FirebaseDatabase.getInstance().getReference().child("Expenses").child(activeUser);
        ...
        pFile = new File(payfile, "ExpensesReport.pdf");
...
//creating an object and setting to displlay
                    ...
                    note = snapshot.child("notes").getValue().toString();
                    amount = snapshot.child("amount").getValue().toString();
                    category = snapshot.child("category").getValue().toString();
                    expenseModel.setNotes(note);
                    expenseModel.setAmount(Integer.parseInt(amount));
                ...
                try {
                    createPaymentReport(expenseList);
    ...
    private void createPaymentReport(  List<ExpenseModel> expenseModelList) throws DocumentException, FileNotFoundException, java.io.FileNotFoundException {
        ...
        Chunk nameText = new Chunk("Category", white);
        ...
        Chunk amountText = new Chunk("Note", white);
        ...
        for (int i = 0; i < expenseModelList.size(); i++) {
            ExpenseModel expenseModel = expenseModelList.get(i);

            String id = String.valueOf(i + 1);
            String category = expenseModel.getCategory();
            String amount = Integer.toString(expenseModel.getAmount());
            String notes = expenseModel.getNotes();
        ...
        document.add(new Paragraph(" ALL EXPENSES REPORT\n\n", g));
        document.add(table);
